...
<title>EduSafe - User Admin Registration</title>
...
<div class="register-container">
    <div class="register-header">
        ...
        <h1>Create User Admin Account</h1>
        <p>Join EduSafe</p>
    </div>

    <div class="register-body">
        <form action="Register_User_AdminServe" method="post" >
			    <div class="row">
			    ...
			    <div class="row">
			        <div class="col-md-6 mb-3">
			            ...
			            <div class="input-with-icon">
			                ...
			                <input type="text" class="form-control" id="teacher_phone" name="teacher_phone" maxlength="12" placeholder="Enter Teacher Phone">
			            </div>
			        </div>
			        <div class="col-md-6 mb-3">
...
<script>
    ...
    function validateForm() {
        ...
        const pattern = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,25}$/;

        if (!pattern.test(password)) {
            passwordError.textContent = "Password must be 8-25 chars, include letters, numbers & special chars.";
            return false;
        }
    ...
    // Append .Admin automatically
    const userIdInput = document.getElementById('user_id');
    userIdInput.addEventListener('blur', () => {
        let val = userIdInput.value.trim();
        if (val && !val.toLowerCase().endsWith('.admin')) {
            userIdInput.value = val + '.Admin';
        }
    });

    document.querySelector('form').addEventListener('submit', (e) => {
        let val = userIdInput.value.trim();
        if (!val.toLowerCase().endsWith('.admin')) {
            userIdInput.value = val + '.Admin';
        }
    });
</script>
